...
if(!identity || !identity.did || identity.did===""){
  ...
  res.json({
   ...
   engine:"Sovereign Identity Engine V77.0"
  });
 } catch(e){console.error(e.stack);res.status(500).json({error:e.message});}
});

app.get("/api/status", (req,res)=>res.json({
  protocol:"Sovereign Identity Protocol SIP v77.0",
  engine:"Sovereign Identity Engine V77.0",
  status:"online",
  runtime:"active",
...
// ===== SIP V45 IDENTITY GRAPH INTELLIGENCE LAYER =====

const identityGraph={};

function buildIdentityGraph(address){
 ...
 return identityGraph[id];
}


function calculateGraphScore(address){

 const graph=buildIdentityGraph(address);

 let score=50;

 score += graph.interactions * 5;
 score += graph.trustNodes * 10;

 if(graph.connections.length>5)
  score+=20;
...
app.get("/api/graph/:address",(req,res)=>{
 ...
 const result=
 calculateGraphScore(req.params.address);
